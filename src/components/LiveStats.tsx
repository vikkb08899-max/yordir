...
const LiveStats: React.FC = () => {
  ...
  useEffect(() => {
    const generateTransaction = (): Transaction => {
      const cryptos = ['TRX', 'USDT', 'BTC', 'ETH', 'USDC'];
      const fiats = ['EUR'];
      ...
      if (isCryptoToFiat) {
        ...
        return {
          ...
          from: crypto,
          to: 'EUR',
          amount: Math.random() * 249000 + 1000, // 1k - 250k EUR equivalent
          timestamp: new Date()
        };
      } else {
        // Fiat to Crypto
        ...
        return {
          ...
          from: 'EUR',
          to: crypto,
          amount: Math.random() * 249000 + 1000, // 1k - 250k EUR
          timestamp: new Date()
        };
      }
    };
    ...
    <div id="stats" className="bg-gray-900/80 backdrop-blur-lg rounded-2xl border border-gray-800 p-6">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">Real-time platform activity</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {transactions.map((tx) => (
            <div 
              key={tx.id}
              className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg hover:bg-gray-800/50 transition-colors animate-fadeIn"
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400">
                    {tx.from === 'EUR' ? 'Fiat → Crypto' : 'Crypto → Fiat'} • {tx.timestamp.toLocaleTimeString()}
                  </div>
                </div>
              </div>
